...
instrument_before_bb_first_i(bb_key_t new_key, slot_t num)
{
    ...
    if (bb_entry_num % 1000 < 30) {
        DRCCTLIB_PRINTF("[r] bb_entry_num %d", bb_entry_num);
        DRCCTLIB_PRINTF("[r] pre key %d, slot_max %d, cur_slot %d, pre_bb_end_state %d",
                        pt->cur_bb_node->key, pt->cur_bb_node->max_slots, pt->cur_slot,
                        pt->pre_bb_end_state);
    ...
    {
        new_caller_ctxt =
            pt->root_bb_node->child_ctxt_start_idx +
            THREAD_ROOT_SHARDED_CALLEE_INDEX;
    } else if (instr_state_contain(pt->pre_bb_end_state, INSTR_STATE_CALL_DIRECT)
    ||
                instr_state_contain(pt->pre_bb_end_state,
        ...
        if(pt->cur_slot >= pt->cur_bb_node->max_slots){
            ...
            DRCCTLIB_PRINTF("???????????");
            DRCCTLIB_EXIT_PROCESS(" > pre bb key %d caller_ctxt_hndl %d",
            pre_bb->key, pre_bb->caller_ctxt_hndl);
        }
