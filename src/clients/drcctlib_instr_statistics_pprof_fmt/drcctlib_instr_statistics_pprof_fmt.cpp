...
ClientExit(void)
{
    ...
    PprofProfile::profile_t* profile = new PprofProfile::profile_t();
    profile->add_sample_type("times", "instruction execute times");
    for (int32_t i = 0; i < TOP_REACH_NUM_SHOW; i++) {
        if (output_list[i].handle == 0) {
            break;
        }
        context_t* cur_ctxt = drcctlib_get_full_cct(output_list[i].handle);
        PprofProfile::sample_t *sample = profile->add_sample(cur_ctxt);
        sample->append_value((int64_t)output_list[i].count);
        drcctlib_free_full_cct(cur_ctxt);
    }
    profile->serialize_to_file("instr_statistics.pprof");
    delete profile;
