...
{
    if (addr > pt->stack_config.stack_end &&
        addr < pt->stack_config.stack_base) {
        pt->number2 ++;
    }
    pt->number1 ++;
    pt->cur_mem_idx ++;
...
ClientExit(void)
{
    ...
    if (!drmgr_unregister_thread_init_event(ClientThreadStart) ||
        ...
        DRCCTLIB_PRINTF("ERROR: drcctlib_stack_memory_rate failed to unregister in ClientExit");
    }
    drmgr_exit();
...
dr_client_main(client_id_t id, int argc, const char *argv[])
{
    ...
    if (!drmgr_init()) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_stack_memory_rate unable to initialize drmgr");
    }
    drmgr_priority_t thread_init_pri = {sizeof(thread_init_pri),
                                                "drcctlib_stack_memory_rate-thread_init",
                                                NULL, NULL, DRCCTLIB_THREAD_EVENT_PRI + 1};
    drmgr_priority_t thread_exit_pri = {sizeof(thread_exit_pri),
                                                "drcctlib_stack_memory_rate-thread-exit",
                                                NULL, NULL, DRCCTLIB_THREAD_EVENT_PRI + 1};
    ...
    if (tls_idx == -1) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_stack_memory_rate drmgr_register_tls_field fail");
    }
