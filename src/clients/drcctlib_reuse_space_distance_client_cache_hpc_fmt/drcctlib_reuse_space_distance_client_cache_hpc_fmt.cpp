...
#define DRCCTLIB_PRINTF(_FORMAT, _ARGS...) \
    DRCCTLIB_PRINTF_TEMPLATE("reuse_space_distance_client_cache_hpc_fmt", _FORMAT, ##_ARGS)
#define DRCCTLIB_EXIT_PROCESS(_FORMAT, _ARGS...)                                          \
    DRCCTLIB_CLIENT_EXIT_PROCESS_TEMPLATE("reuse_space_distance_client_cache_hpc_fmt", _FORMAT, \
                                          ##_ARGS)
...
UpdateUseAndReuseMap(void *drcontext, per_thread_t *pt, mem_ref_t *ref, int cur_mem_idx)
{
    ...
    if (it != (*use_map).end()) {
        ...
        data_handle_t data_hndl =
            drcctlib_get_data_hndl_ignore_stack_data(drcontext, (app_pc)real_data_addr);
...
ClientExit(void)
{
    if (!dr_raw_tls_cfree(tls_offs, INSTRACE_TLS_COUNT)) {
        DRCCTLIB_EXIT_PROCESS(
            "ERROR: drcctlib_reuse_space_distance_client_cache_hpc_fmt dr_raw_tls_calloc fail");
    }

    if (!drmgr_unregister_thread_init_event(ClientThreadStart) ||
        !drmgr_unregister_thread_exit_event(ClientThreadEnd) ||
        !drmgr_unregister_tls_field(tls_idx)) {
        DRCCTLIB_PRINTF("ERROR: drcctlib_reuse_space_distance_client_cache_hpc_fmt failed to "
...
{
    dr_set_client_name("DynamoRIO Client 'drcctlib_reuse_space_distance_client_cache_hpc_fmt'",
                       "http://dynamorio.org/issues");
    ClientInit(argc, argv);

    if (!drmgr_init()) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_reuse_space_distance_client_cache_hpc_fmt unable "
                              "to initialize drmgr");
    }
    ...
    if (tls_idx == -1) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_reuse_space_distance_client_cache_hpc_fmt "
