...
dr_client_main(client_id_t id, int argc, const char *argv[])
{
    ...
    if (!drmgr_init()) {
        DRCCTLIB_EXIT_PROCESS("WARNING: drcctlib_overhead_test unable to initialize drmgr");
    }
    if (!drmgr_register_bb_instrumentation_event(event_bb_analysis,
                                                 event_bb_insert, NULL)) {
        DRCCTLIB_EXIT_PROCESS("WARNING: drcctlib_overhead_test fail to register bb instrumentation event");
    }
