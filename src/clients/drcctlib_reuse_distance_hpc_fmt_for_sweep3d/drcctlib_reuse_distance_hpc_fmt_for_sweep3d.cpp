...
#define DRCCTLIB_EXIT_PROCESS(format, args...)                                         \
    do {                                                                               \
        ...
        gethostname(name + strlen(name), MAXIMUM_PATH - strlen(name));                 \
        pid_t pid = getpid();                                                          \
        dr_printf("[(%s%d)drcctlib_reuse_distance_hpc_fmt_for_sweep3d(%s%d) msg]====" format "\n", \
                  name, pid, ##args);                                                  \
    } while (0);                                                                       \
    dr_exit_process(-1)
...
UpdateUseAndReuseMap(void* drcontext, per_thread_t *pt, mem_ref_t * ref, int cur_mem_idx)
{
    ...
    if (it != (*use_map).end()) {
        ...
        data_handle_t data_hndl = drcctlib_get_data_hndl_ignore_stack_data(drcontext, (app_pc)real_data_addr);
...
ClientExit(void)
{
    ...
    if (!dr_raw_tls_cfree(tls_offs, INSTRACE_TLS_COUNT)) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_reuse_distance_hpc_fmt_for_sweep3d dr_raw_tls_calloc fail");
    } 

    if (!drmgr_unregister_thread_init_event(ClientThreadStart) ||
        !drmgr_unregister_thread_exit_event(ClientThreadEnd) ||
        !drmgr_unregister_tls_field(tls_idx)) {
        DRCCTLIB_PRINTF("ERROR: drcctlib_reuse_distance_hpc_fmt_for_sweep3d failed to unregister in ClientExit");
    }
    drmgr_exit();
...
{
    dr_set_client_name("DynamoRIO Client 'drcctlib_reuse_distance_hpc_fmt_for_sweep3d'",
                       "http://dynamorio.org/issues");
    ClientInit(argc, argv);

    if (!drmgr_init()) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_reuse_distance_hpc_fmt_for_sweep3d unable to initialize drmgr");
    }
    drreg_options_t ops = { sizeof(ops), 3 /*max slots needed*/, false };
    if (drreg_init(&ops) != DRREG_SUCCESS) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_reuse_distance_hpc_fmt_for_sweep3d unable to initialize drreg");
    }
    if (!drutil_init()) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_reuse_distance_hpc_fmt_for_sweep3d unable to initialize drutil");
    }
    ...
    if (tls_idx == -1) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_reuse_distance_hpc_fmt_for_sweep3d drmgr_register_tls_field fail");
    }
    if (!dr_raw_tls_calloc(&tls_seg, &tls_offs, INSTRACE_TLS_COUNT, 0)) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_reuse_distance_hpc_fmt_for_sweep3d dr_raw_tls_calloc fail");
    }
    drcctlib_init_ex(DRCCTLIB_FILTER_MEM_ACCESS_INSTR, INVALID_FILE, InstrumentInsCallback, NULL,
                    NULL, NULL, NULL, NULL, NULL, NULL, DRCCTLIB_SAVE_HPCTOOLKIT_FILE | DRCCTLIB_COLLECT_DATA_CENTRIC_MESSAGE);
