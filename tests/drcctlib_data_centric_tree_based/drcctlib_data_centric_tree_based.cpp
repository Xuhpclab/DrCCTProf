...
void MemAnalysisRoutine(const uint32_t slot) {
    // add code here
    // GetDataObjectHandle(dr_get_current_drcontext(), NULL);
}
...
void InstrumentInsCallback(void *drcontext, instrlist_t *ilist, instr_t* ins, void * v, uint slot) {
    ...
    if(instr_writes_memory(ins) || instr_reads_memory(ins)){
        dr_insert_clean_call(drcontext, ilist, ins, (void *)MemAnalysisRoutine, false, 1, OPND_CREATE_INT32(slot));
    }
}
...
dr_client_main(client_id_t id, int argc, const char *argv[])
{
    ...
    drcctlib_init(INTERESTING_INS_ALL, gTraceFile, InstrumentInsCallback, 0, 0, true);
