...
ClientExit(void)
{
    ...
    if (!drmgr_unregister_thread_init_event(ClientThreadStart) ||
        ...
        DRCCTLIB_PRINTF("ERROR: drcctlib_cct_only failed to unregister in ClientExit");
    }
    drmgr_exit();
...
dr_client_main(client_id_t id, int argc, const char *argv[])
{
    ...
    if (!drmgr_init()) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_cct_only unable to initialize drmgr");
    }
    drreg_options_t ops = { sizeof(ops), 4 /*max slots needed*/, false };
    if (drreg_init(&ops) != DRREG_SUCCESS) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_cct_only unable to initialize drreg");
    }
    if (!drutil_init()) {
        DRCCTLIB_EXIT_PROCESS("ERROR: drcctlib_cct_only unable to initialize drutil");
    }
