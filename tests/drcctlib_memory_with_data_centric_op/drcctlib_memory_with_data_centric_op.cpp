...
#define DRCCTLIB_PRINTF(format, args...)                                            \
    do {                                                                            \
        ...
        pid_t pid = getpid();                                                       \
        dr_printf("[(%s%d)drcctlib_memory_with_data_centric_op msg]====" format "\n", name, pid, \
                  ##args);                                                          \
    } while (0)
...
#define DRCCTLIB_EXIT_PROCESS(format, args...)                                       \
    do {                                                                             \
        ...
        gethostname(name + strlen(name), MAXIMUM_PATH - strlen(name));               \
        pid_t pid = getpid();                                                        \
        dr_printf("[(%s%d)drcctlib_memory_with_data_centric_op(%s%d) msg]====" format "\n", name, \
                  pid, ##args);                                                      \
    } while (0);                                                                     \
    dr_exit_process(-1)
...
{
    dr_set_client_name("DynamoRIO Client 'drcctlib_memory_with_data_centric_op'",
